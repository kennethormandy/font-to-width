 ...
 *
 * © 2016 Kenneth Ormandy
 * © 2014–2015 Chris Lewis http://chrissam42.com and Nick Sherman http://nicksherman.com
...
var FontToWidth = function (options) {
  ...
  forEach(this.allTheElements, function (el) {
    ...
    el.setAttribute('ftw-original-style', el.getAttribute('style'))
  })

  this.measureFonts.bind(this)()
}
...
FontToWidth.prototype.measureFonts = function () {
  ...
  styleEl.id = 'ftw-adobe-blank'
  styleEl.innerHTML = '@font-face { font-family: AdobeBlank src: url("data:application/vnd.ms-fontobjectbase64,' + ftw.eotData + '") format("embedded-opentype"), url("data:application/font-woffbase64,' + ftw.woffData + '") format("woff") }'
  ftw.debug(styleEl)
  document.head.appendChild(styleEl)

  // create a hidden element to measure the relative widths of all the fonts
  // var div = ftw.measure_div = $('<div style=""></div>')
  divHidden.style.position = 'absolute'
  divHidden.style.top = 0
  divHidden.style.right = '101%'
  ...
  forEach(ftw.options.fonts, function (font, i) {
    ...
    span.style.fontWeight = font.fontWeight || 'normal'
    span.style.fontStyle = font.fontStyle || 'normal'

    // First load up a default font
    // span.setAttribute('font-family', span.style.fontFamily)
    span.setAttribute('font-family', font.fontFamily)
    ...
    // div.append(span)
    // div.append('<br>')
    divHidden.appendChild(span)
    divHidden.appendChild(document.createElement('br'))
  })
  ...
  var tries = ftw.options.attempts
  // var spans = ftw.measure_div.children('span')
  var spans = ftw.measure_div.querySelectorAll('span')
  ...
  var measurefunc = function () {
    ...
    ftw.debug('Measured', Date.now()/1000, ftw.fontwidths)
  ...
  setTimeout(function () {
    forEach(spans, function (span, i) {
      ...
      span.style.fontFamily = span.getAttribute('font-family') + ', AdobeBlank'
    })
...
FontToWidth.prototype.startTheBallRolling = function () {
  var ftw = this
  ftw.debug('start the ball rolling')
...
FontToWidth.prototype.updateWidths = function () {
  ...
  var updateSingleWidth = function (el, i) {
    ...
    if (letterspace >= ftw.options.minLetterSpace || newfontsize != oldfontsize || cell.classList.contains('ftw_final')) {
  ...
  forEach(ftw.options.fonts, function (font, i) {
    ...
    forEach(ftw.stillToDo, function (el) {
      ...
      if (font) {
        ...
        el.style.fontWeight = font.fontWeight || 'normal'
    ...
    ftw.debug(font, ftw.stillToDo.length + ' left.')
  ...
  ftw.debug('Widths updated in ' + ((endtime-starttime)/1000) + 's')
}

FontToWidth.prototype.debug = function () {
  var ftw = this

  if (ftw.options.debug === true) {
